...
│   ├── demo_feature_scaling.py
│   └── eda_phase2.py          <- Tasks 2.1-2.4  distribution, volatility,
│                                  correlation, emission-distribution EDA
│
├── data/
│   ├── raw/                   <- unmodified download
│   └── processed/             <- derived features (the model's input)
│                                  + regimes.csv, the decoded output
│
├── models/                    <- serialised trained model (joblib)
├── reports/figures/           <- figures for the slide deck
├── slides/                    <- Phase 5 deck (HMM_Regime_Detection.pptx)
├── tests/                     <- pytest suite
